...
function droneMotion(allowStop, action, type, duration) {
    var timeoutInterval = 2;
    var faye;
    faye = new Faye.Client("/faye", {
        timeout: 120
    });
    if (allowStop == true) {
        var timeout = setTimeout(stopMotion, timeoutInterval * 1000);
    }
...
function ledAnimate() {
    return faye.publish("/drone/animateLeds", {
        action: 'snakeGreenRed',
        freq: 5,
        duration:2
    });
}
