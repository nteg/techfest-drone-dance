            ...
            recognition.onresult = function (event) {
                ...
                for (var j = 0, l = commandsList.length; j < l; j++) {
                    ...
                    if (result != null) {
                        ...
                        //anubhav debug
                        command_span.innerHTML = commandsList[j].originalPhrase;
                        animateSpeechCommand();
                        commandIndex = j;
                        if (debugState) {
                        ...
                        $.each(keywords, function (index, value) {
                            ...
                            if (result) {
                                var parameters = result.slice(1);
                                //anubhav debug
                                command_span.innerText = commandsList[j].originalPhrase;
                                animateSpeechCommand();
                                commandIndex = j;
                                if (debugState) {
                                    root.console.log('command matched: %c' + commandsList[j].originalPhrase, debugStyle);
            ...
            for (var j = 0, l = commandsList.length; j < l; j++) {
                ...
                $.each(keywords, function (index, value) {
                    ...
                    if (result) {
                        var parameters = result.slice(1);
                        //anubhav debug
                        command_span2.innerText = commandsList[j].originalPhrase;
                        animateCommand();
                        commandIndex = j;
                        //if (debugState) {
                        //    root.console.log('command matched: %c' + commandsList[j].originalPhrase, debugStyle);
                        ...
                        // execute the matched command
                        if (commandIndex != -1) {
                            commandsList[j].callback.apply(recognition, parameters);
                            invokeCallbacks(callbacks.resultMatch);
                            return false;
                        }
                    }
